...
|This realm takes in any Oidc/OAuth credentials found on the incoming Authentication Token, and if so does its best to resolve an ID_Token using said credentials. The ID_Token is then used to put together the resulting Authentication Info.
